...
export function Navbar() {
  ...
  debounce(async (term: string) => {
    ...
    try {
      ...
      // Aqui chamamos o endpoint genérico de busca
      ...
      // O array de produtos está em res.data.data
    ...
    const handleKeyDown = (e: KeyboardEvent<HTMLInputElement>) => {
        const list = searchResults.length
            ...
            : history.map((h) => ({ id: h, slug: "", name: h, images: [], price_per: undefined }));
        if (e.key === "ArrowDown") {
            e.preventDefault();
        ...
        if (e.key === "Enter" && selectedIndex >= 0) {
            ...
            const term = "slug" in item && item.slug ? item.slug : item.name;
            submitSearch(term);
        }
    };

    // Dispara a navegação e salva no histórico
    ...
    return (
        <header className="relative w-full top-0 z-50">
            {/* Linha superior: mobile menu, logo, busca desktop e ícones */}
            <div
            ...
            >
                ...
                {/* Busca desktop */}
                <div
                    className="hidden md:block flex-1 max-w-lg mx-8 relative"
                    ref={searchRef}
                >
                    <input
                        ref={inputRef}
                    ...
                    {/* Dropdown de histórico ou resultados */}
                    {(showHistory && history.length > 0) ||
                        ...
                        <div className="absolute top-full left-0 w-full bg-white rounded-lg shadow-lg mt-1 z-50 max-h-64 overflow-auto">
                                ...
                                searchResults.length === 0 && (
                                    <div className="p-2">
                                        <div className="flex justify-between items-center mb-2">
                                            <span className="font-medium text-gray-500">Histórico de buscas</span>
                                            <button
                                        ...
                                        {history.map((h, idx) => (
                                            <div
                                                key={h}
                                                className={`px-3 py-2 hover:bg-gray-100 text-black cursor-pointer ${selectedIndex === idx ? "bg-gray-200" : ""
                                                    }`}
                            ...
                            {searchResults.map((prod, idx) => (
                                <div
                                    key={prod.id}
                                    onMouseDown={() => submitSearch(prod.name)}   // <— grava no histórico
